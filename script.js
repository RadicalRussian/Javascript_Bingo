...
// The function draw() will draw a number of cards dependon the calue given by the user
function draw(){
    //Condition if the user click the draw button
    // without existing values in field.
    if(txtInput.value ==""){
        alert("Enter value in the field!");
    }
    else{
            for(var i= 0; i <txtInput.value; i++){
                // create object of BingoCards class
                card = new BingoCard();
                card.generateMatrix();
    ...
    for(var i=0; i< cardMatrixTotal.length; i++){
        for(var j= 0; j< cardMatrixTotal[i].length; j++){
            for(var k=0; k < cardMatrixTotal[i][j].length; k++){
                ...
                //highlights the nuber matched form drawll into the bingo card
...
function showWinnerCard(){
    //user confirmation to display winning card
...
function clearCards(){
    ...
    let confirmation= confirm("Do you want to clear/restart game?");
    if(confirmation = true){
...
function checkBingoWinner(){
    ...
    //check if the bingo winner is horizontal
